...
$product = mysqli_real_escape_string($conn, $_POST['firstname']);
...
$select = "SELECT * FROM signup where product_name ='$product_name' && price = '$price'";
...
$sql= "SELECT images.product_id, signup.firstname, signup.lastname, images.product_name, images.description, images.price
        FROM signup
        INNER JOIN images ON signup.user_id = images.product_id";
$result= $conn->query($sql);
if($result->num_rows > 0){
    //output data of each row
    while($row= $result-> fetch_assoc()){
        echo "<tr><td>".$row["product_id"]. "</td><td>" . $row["firstname"] . "</td><td>". $row["lastname"]."</td><td>".$row["product_name"]. "</td><td>".$row["description"] . "</td><td>".$row["price"] ."</td><td>". "</td></tr>";
    }
echo"</table>";
}
else{
